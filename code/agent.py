...
# ---------------------------------------------------------------------------
# Hybrid Mode:
# If GEMINI_API_KEY is found, it uses Gemini API (with safe delays & large context).
# If NO key is found, it falls back to Ollama (local, no quota, fast).
# ---------------------------------------------------------------------------
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")
USE_GEMINI = HAS_GEMINI and bool(GEMINI_API_KEY)

OLLAMA_BASE_URL  = os.getenv('OLLAMA_BASE_URL', 'http://localhost:11434')
OLLAMA_MODEL     = os.getenv('OLLAMA_MODEL', 'gemma3:1b')

MAX_CORPUS_CHARS = 40_000 if USE_GEMINI else 6_000
INTER_TICKET_DELAY = 12 if USE_GEMINI else 0
...
class TicketResponse(BaseModel):
    ...
    response: str = Field(description="user-facing answer grounded in the corpus, or polite out-of-scope message")
    justification: str = Field(description="concise explanation of the routing/answering decision")
    request_type: str = Field(description="'product_issue', 'feature_request', 'bug', or 'invalid'")
...
class SupportAgent:
    ...
    def _check_ollama(self):
        try:
            ...
            if not any(OLLAMA_MODEL.split(':')[0] in m for m in models):
                logger.warning("Model '%s' not found locally! Run: ollama pull %s", OLLAMA_MODEL, OLLAMA_MODEL)
        except requests.exceptions.ConnectionError:
            raise RuntimeError("Ollama is not running! Ensure it is installed from ollama.com")
    ...
    def _retrieve_relevant_chunks(self, query: str, company: str, max_chars: int = MAX_CORPUS_CHARS):
        stop_words = {'the', 'and', 'for', 'that', 'this', 'with', 'have', 'from',
                      'are', 'was', 'not', 'but', 'they', 'you', 'your', 'our',
                      'can', 'will', 'how', 'what', 'when', 'where', 'which', 'who',
                      'their', 'there', 'been', 'has', 'had', 'its', 'also', 'all'}
        tokens = set(w.lower() for w in re.findall(r'\b[a-zA-Z]{3,}\b', query) if w.lower() not in stop_words)

        companies_to_search = [company] if company in self.chunks else list(self.chunks.keys())
        ...
        for attempt in range(max_retries):
            try:
                ...
                if '429' in err or 'RESOURCE_EXHAUSTED' in err or '503' in err or 'UNAVAILABLE' in err:
                    # Exponential backoff
                    try:
                        delay_match = re.search(r'retry.*?(\d+)\.?\d*s', err, re.IGNORECASE)
                        parsed_wait = int(delay_match.group(1)) if delay_match else 0
                        backoff = min(15 * (2 ** attempt), 60)
                        wait = max(parsed_wait + 5, backoff) if parsed_wait > 0 else backoff
                    except Exception:
    ...
    def _process_with_ollama(self, user_prompt, system_prompt):
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.warning("Ollama error (attempt %d): %s", attempt + 1, str(e)[:100])
                time.sleep(3)
